...
class LocalMirror {
  ...
  Future<String?> uploadNewImagesOrFiles<DataT extends Data>(
    ...
    // Copy sequentially and at file-system level. Reading an entire batch of
    // full-resolution photos into Dart memory at once caused large transient
    // allocations and Android low-memory terminations.
    for (final file in files) {
      final imageName = _scoped(canonicalTimestampFilenameForXFile(file));
      final target = await OP.localFile(imageName);
      await target.parent.create(recursive: true);
      final sourcePath = File(file.path).absolute.path;
      final targetPath = target.absolute.path;
      final targetIsValid = target.existsSync() && target.lengthSync() >= 5;
      if (!targetIsValid && sourcePath != targetPath) {
