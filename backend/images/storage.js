...
const mstorage = multer.diskStorage({
  ...
  destination: (req, file, cb) => {
    ...
    rootfolder(req.body.data).then((rf) => {
      ...
      if (set_first_image_as_main && rf.mainfilename == no_image_placeholder_name)
        fs.readdir(path, {}, (err, files) => {
          rf.mainfilename = file.originalname;
          if (files.length < (1 + files.includes(rf.mainfilename))) { //lil race condition workaround: if file already added length is increased by 1
            // console.log(files)
